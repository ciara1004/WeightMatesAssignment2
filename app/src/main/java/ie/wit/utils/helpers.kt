...
fun createLoader(activity: FragmentActivity) : AlertDialog {
    ...
    loader.setIcon(R.mipmap.ic_launcher_weightmates_foreground)
...
fun uploadImageView(app: WeightMatesApp, imageView: ImageView) {
    ...
    uploadTask.addOnFailureListener { object : OnFailureListener {
        override fun onFailure(error: Exception) {
            Log.v("Calorie", "uploadTask.exception" + error)
        }
    }
...
fun updateAllCalories(app: WeightMatesApp) {
    val userId = app.auth.currentUser!!.uid
    val userEmail = app.auth.currentUser!!.email
    var calorieRef = app.database.ref.child("calories")
                                  .orderByChild("email")
    val usercalorieRef = app.database.ref.child("user-calories")
                                  .child(userId).orderByChild("uid")

    calorieRef.equalTo(userEmail).addListenerForSingleValueEvent(
    ...
    usercalorieRef.addListenerForSingleValueEvent(
        object : ValueEventListener {
        override fun onCancelled(error: DatabaseError) {}
        override fun onDataChange(snapshot: DataSnapshot) {
            snapshot.children.forEach {
...
fun writeImageRef(app: WeightMatesApp, imageRef: String) {
    val userId = app.auth.currentUser!!.uid
...
fun validatePhoto(app: WeightMatesApp, activity: Activity) {
    ...
    {
        activity.navView.getHeaderView(0).imageView.setImageResource(R.mipmap.ic_launcher_weightmates_foreground)
        uploadImageView(app, activity.navView.getHeaderView(0).imageView)
    }
}

fun checkExistingPhoto(app: WeightMatesApp, activity: Activity) {

    app.userImage = "".toUri()
    Log.v("Calorie","checkExistingPhoto 1 app.userImage : ${app.userImage}")
    ...
    app.database.child("user-photos").orderByChild("uid")
        ...
        override fun onDataChange(snapshot: DataSnapshot ) {
            snapshot.children.forEach {
                ...
                Log.v("Calorie","checkExistingPhoto 2 app.userImage : ${app.userImage}")
            }
            Log.v("Calorie","validatePhoto 3 app.userImage : ${app.userImage}")
